        ...
        try {
            ...
            DefaultConsumer defaultConsumer = new DefaultConsumer(channel) {
                ...
                public void handleDelivery(String consumerTag, Envelope envelope, AMQP.BasicProperties properties, byte[] body) throws IOException {
                     ...
                     *            息转发给队列。
                     * 4.topics-通配符模式
                     * ***** 特点：一个交换机可以绑定多个队列，每个队列可以设置一个或多个带通配符的routingkey，生
                     *            产者将消息发给交换机，交换机根据routingkey的值来匹配队列，匹配时采用通配符的方
                     *            式，匹配成功的将消息转发到指定的队列。
                     * 5.header-转发器模式
                     * ***** 特点：header模式与routingkey和topics模式不同的地方在于，header模式取消了routingkey，
                     *            使用header中的key/value键值对匹配队列。
                     * 6.RPC-远程过程调用模式
                     */
